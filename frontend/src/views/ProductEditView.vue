              ...
              <el-dropdown @command="handleUserCommand">
                ...
                <template #dropdown>
                  <el-dropdown-menu>
                    ...
                    <el-dropdown-item command="create-product">发布商品</el-dropdown-item>
...
import { getProductDetail, updateProduct, getCategories, type Product, type Category } from '@/services/product'
...
const loadProduct = async () => {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      // 检查是否是商品拥有者
      if (authStore.user && authStore.user.id !== product.value.boss_id) {
      ...
      productForm.title = product.value.title
      productForm.category_id = product.value.category_id
      productForm.price = parseFloat(product.value.price)
      productForm.discount_price = product.value.discount_price !== product.value.price
        ? parseFloat(product.value.discount_price)
        : undefined
      productForm.num = product.value.num
      productForm.info = product.value.info
      productForm.on_sale = product.value.on_sale
    } else {
...
const submitProduct = async () => {
  if (!productFormRef.value || !product.value) return
  ...
  try {
    ...
    const response = await updateProduct(product.value.id.toString(), submitData)
