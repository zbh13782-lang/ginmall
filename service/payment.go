...
func (service *OrderPay) PayDown(ctx context.Context, uid uint) serializer.Response {
	...
	err := dao.NewOrderDao(ctx).Transaction(func(tx *gorm.DB) error {
		...
		// 对钱进行解密。减去订单。再进行加密。
		moneyStr, _ := utils.Encrypt.AesDecoding(user.Money)
		...
		user.Money, _ = utils.Encrypt.AesEncoding(finMoney)
		...
		moneyStr, _ = utils.Encrypt.AesDecoding(boss.Money)
		moneyFloat, _ = strconv.ParseFloat(moneyStr, 64)
		finMoney = fmt.Sprintf("%f", moneyFloat+money)
		boss.Money, _ = utils.Encrypt.AesEncoding(finMoney)
